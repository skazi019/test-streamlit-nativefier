...
class CalculateProfitLoss:
    ...
    def calculate_pl(ticker_df):
        ...
        ticker_df = ticker_df[
            (ticker_df["SIGNAL"] == "BUY") | (ticker_df["SIGNAL"] == "SELL")
        ]
        ...
        ticker_df.loc[:, "BACKTEST_PL"] = (
            ticker_df["Close"].diff(periods=1).mask(ticker_df["SIGNAL"] == "BUY").copy()
        )
        ...
        print(
            f"Winning Profit: {sum(ticker_df[ticker_df['BACKTEST_PL'] > 0]['BACKTEST_PL'])}"
        )
        print(f"Losing Trades: {len(ticker_df[ticker_df['BACKTEST_PL'] < 0])}")
        print(
            f"Losing Losses: {sum(ticker_df[ticker_df['BACKTEST_PL'] < 0]['BACKTEST_PL'])}"
        )
